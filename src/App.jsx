...
export default function ChessClub() {
  ...
  const unresolvedTiebreakPairs = useMemo(() => {
    ...
    for (let i = 0; i < standings.length - 1; i++) {
      ...
      if (stillTied) {
        // Skip if a tiebreaker decider for this exact pair already exists (pending or resolved)
        const alreadyExists = (tiebreakers || []).some(tb =>
          (tb.playerAId === a.id && tb.playerBId === b.id) || (tb.playerAId === b.id && tb.playerBId === a.id)
        );
        if (!alreadyExists) pairs.push({ playerAId: a.id, playerBId: b.id, playerAName: a.name, playerBName: b.name, rank: i + 1 });
      }
    }
    return pairs;
      ...
      <div style={s.body}>
        ...
        {tab === "Schedule" && (
          <>
            ...
            {(() => {
              const noRoundMatches = matches.filter(m => !m.round && !results.find(r => r.matchId === m.id));
