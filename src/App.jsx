...
export default function ChessClub() {
  ...
  const buildRoundRobinSchedule = () => {
    ...
    pairingRounds.forEach((roundPairs, roundIdx) => {
      ...
      roundPairs.forEach(([white, black]) => {
        newMatches.push({
          id: matchIdCounter++,
          whiteId: white.id,
          blackId: black.id,
        ...
        {tab === "Schedule" && (
          <>
            ...
            <div style={s.card}>
              ...
              {(() => {
                // Group matches by round number; matches without a round (manually scheduled
                // outside the auto-generated season) are bucketed separately at the end.
                const rounds = {};
                const noRound = [];
                notStartedMatches.forEach(m => {
                  if (m.round) {
                  ...
                  <>
                    {roundNumbers.map(rn => (
                      <div key={rn} style={{ marginBottom: 14 }}>
                        <div style={{ fontSize: 12, fontWeight: 800, color: "#7c3aed", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 6, marginTop: 10 }}>
                          🔄 Round {rn}
                        </div>
                        {rounds[rn].map(MatchRow)}
                      </div>
                    ))}
                    {noRound.length > 0 && (
                      <div>
                        {roundNumbers.length > 0 && (
                          <div style={{ fontSize: 12, fontWeight: 800, color: T.textMuted, letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 6, marginTop: 10 }}>
                            Other Matches
                          </div>
                        )}
                        {noRound.map(MatchRow)}
                      </div>
                    )}
                  </>
                );
