...
import { getFirestore, doc, setDoc, onSnapshot, getDoc, collection, updateDoc, deleteDoc } from "firebase/firestore";
import { getAuth, signInWithEmailAndPassword, createUserWithEmailAndPassword, signOut, onAuthStateChanged } from "firebase/auth";
import { getDatabase, ref, set, onDisconnect, onValue, serverTimestamp, push, remove } from "firebase/database";
...
// Generates a full double round-robin schedule (everyone plays everyone twice, colors swapped
// on the rematch) using the standard "circle method": one player stays fixed while the rest
// rotate around them each round, guaranteeing every pairing happens exactly once per half-season.
// Requires an even number of players (no byes).
function generateRoundRobinPairings(players) {
  const n = players.length;
  if (n < 2 || n % 2 !== 0) return []; // caller should validate evenness before calling

  const half = n / 2;
  const rotation = players.slice(1); // everyone except the fixed first player
  const firstHalfRounds = []; // array of rounds, each round is an array of [playerA, playerB]

  for (let round = 0; round < n - 1; round++) {
    const roundPairs = [];
    const fixed = players[0];
    const rotated = [fixed, ...rotation];
    ...
    // Rotate: take the last element of `rotation` and move it to the front
    rotation.unshift(rotation.pop());
  }

  // Second half: same pairings, but swap which player is "home" (White) — i.e. reverse each pair.
  const secondHalfRounds = firstHalfRounds.map(roundPairs => roundPairs.map(([a, b]) => [b, a]));

  return [...firstHalfRounds, ...secondHalfRounds]; // total 2*(n-1) rounds
}

// Given a start date, alternates Saturday/Tuesday for each successive round, starting from the
// next upcoming Saturday (or today if today is already Saturday).
function getRoundDates(roundCount, fromDate = new Date()) {
  const dates = [];
  const d = new Date(fromDate);
  // Move forward to the next Saturday (day 6). If today is Saturday, use today.
  ...
  for (let i = 0; i < roundCount; i++) {
    ...
    if (i % 2 === 0) {
      d.setDate(d.getDate() + 3); // Saturday -> following Tuesday
    } else {
      d.setDate(d.getDate() + 4); // Tuesday -> following Saturday
    }
  }
  return dates;
}
...
// Assigns a time (9:00 PM or 10:00 PM) to every match in the season. Within a round, matches
// alternate 9pm/10pm by their order. When a pairing's REMATCH happens (second half of the
// season), it gets the opposite slot from their first meeting — e.g. if Amin vs Khadar played
// at 9pm in round 1, their rematch (Khadar vs Amin) gets 10pm later in the season.
function assignTimeSlots(pairingRounds) {
  const TIMES = ["21:00", "22:00"]; // 9 PM, 10 PM in 24h format for <input type="time">
  const half = pairingRounds.length / 2; // first-half round count == second-half round count
  const firstHalfSlots = []; // firstHalfSlots[roundIdx] = array of slot indices (0 or 1) per match in that round

  // First half: simple alternation by match order within each round
  for (let r = 0; r < half; r++) {
    const roundPairs = pairingRounds[r];
    firstHalfSlots.push(roundPairs.map((_, i) => i % 2));
  }

  const allSlots = [...firstHalfSlots];
  // Second half: same round structure (mirrored), but each match's slot is the OPPOSITE of
  // its first-meeting slot, since pairingRounds' second half already has pairs reversed
  // ([a,b] -> [b,a]) at the same round-index + half offset.
  for (let r = 0; r < half; r++) {
    const flipped = firstHalfSlots[r].map(slot => 1 - slot);
    allSlots.push(flipped);
  }

  return allSlots; // allSlots[roundIdx] = array of slot indices (0=9pm, 1=10pm) per match in that round
}
...
// Spectator emoji reactions for a live match live in their own RTDB path so the high-frequency
// reaction writes don't collide with move updates on the same liveGames/{matchId} node.
const reactionsRef = (matchId) => ref(rtdb, `reactions/${matchId}`);
    ...
    try {
      if (mode === "register") {
        ...
        await setDoc(doc(db, "users", uid), {
          ...
          permissions: isHeadAdmin ? { addPlayers: true, removePlayers: true, scheduleMatches: true, enterResults: true, startMatches: true, cancelMatches: true, postAnnouncements: true, deleteAnnouncements: true, viewActivity: true, sendInviteCodes: true, endSeason: true } : DEFAULT_MOD_PERMISSIONS,
      ...
      <div style={s.box}>
        ...
        {mode === "register" && (
          <div style={{ marginBottom: 12 }}>
            ...
            <input style={s.input} placeholder="Ask your Head Admin" value={inviteCode} onChange={e => { setInviteCode(e.target.value); setError(""); }} />
...
function AdminPanel({ db, currentUser, HEAD_ADMIN_EMAIL, DEFAULT_MOD_PERMISSIONS, players, setPlayers, T, s }) {
  ...
  const [confirmRemoveUid, setConfirmRemoveUid] = React.useState(null);
  ...
  // Revokes a user's app access by deleting their Firestore profile (users/{uid}).
  // NOTE: this does NOT delete their Firebase Authentication login — Firebase blocks
  // client-side deletion of other users' Auth accounts for security reasons. To remove
  // the login itself, go to Firebase Console -> Authentication -> Users and delete it there.
  // If this user had claimed a player profile via invite code, that profile is unlinked
  // and given a fresh invite code so it can be claimed again by the real player.
  const removeUser = async (user) => {
    setRemovingUid(user.uid);
    try {
      await deleteDoc(doc(db, "users", user.uid));
      if (user.playerId && players && setPlayers) {
        const fresh = players.find(p => p.id === user.playerId);
        if (fresh) {
          setPlayers(prev => prev.map(p =>
            p.id === user.playerId ? { ...p, claimedBy: null, inviteCode: generateInviteCode(p.name) } : p
          ));
        }
      }
  ...
  const permLabels = {
    ...
    enterResults: "Enter Results (Manual Override)",
    ...
    sendInviteCodes: "Send Invite Codes / WhatsApp Chat",
  ...
  return (
    <div>
      <div style={s.card}>
        ...
        {loadError && (
          <div style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, padding: "12px 16px", color: "#ef4444", fontSize: 12 }}>
            ⚠️ Couldn't load users: {loadError}<br />
            This is usually a Firestore security rules issue — make sure the "users" collection allows authenticated reads/writes in Firebase Console → Firestore Database → Rules.
          </div>
        )}
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                {user.email !== HEAD_ADMIN_EMAIL && (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6, alignItems: "flex-end" }}>
                    ...
                    <button onClick={() => setConfirmRemoveUid(user.uid)} style={{ background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#ef4444", borderRadius: 8, padding: "6px 12px", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>Remove</button>
                  </div>
                )}
              </div>
            ) : confirmRemoveUid === user.uid ? (
              <div>
                <div style={{ fontWeight: 700, fontSize: 14, color: "#ef4444", marginBottom: 8 }}>Remove {user.name}'s access?</div>
                <div style={{ fontSize: 12, color: T.textMuted2, marginBottom: 12, lineHeight: 1.5 }}>
                  This deletes their app profile and revokes their role/permissions immediately. {user.playerId ? "Their linked player profile will be unlinked and given a fresh invite code. " : ""}
                  Their login (email/password) still exists in Firebase — if you also want to block them from logging in at all, delete them in Firebase Console → Authentication → Users.
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button style={{ ...s.btn("#ef4444"), marginTop: 0, flex: 1 }} onClick={() => removeUser(user)} disabled={removingUid === user.uid}>
                    {removingUid === user.uid ? "Removing..." : "🗑️ Confirm Remove"}
                  </button>
                  <button style={{ ...s.btn("#374151"), marginTop: 0, flex: 1 }} onClick={() => setConfirmRemoveUid(null)}>Cancel</button>
                </div>
              </div>
            ) : (
              <div>
...
function LiveBoard({ matchId, match, myColor, onClose, T, getPlayerPhoto, onGameEnd }) {
  ...
  const [showTiebreakerIntro, setShowTiebreakerIntro] = useState(!!match.isTiebreaker);
  const [floatingReactions, setFloatingReactions] = useState([]); // [{ id, emoji, left }] currently animating on screen
  ...
  // Listens for new spectator reactions and animates each one floating up the screen, then
  // auto-removes it from local state after the animation finishes. Reactions are ephemeral —
  // they're written with push() so every tap gets a unique key, and each reaction deletes
  // itself from RTDB a couple seconds after being sent so the node never grows unbounded.
  const seenReactionIds = useRef(new Set());
  useEffect(() => {
    const rref = reactionsRef(matchId);
    const unsub = onValue(rref, (snap) => {
      const val = snap.val();
      if (!val) return;
      Object.entries(val).forEach(([id, data]) => {
        if (seenReactionIds.current.has(id)) return;
        seenReactionIds.current.add(id);
        const floatId = `${id}_${Math.random()}`;
        setFloatingReactions(prev => [...prev, { id: floatId, emoji: data.emoji, left: 10 + Math.random() * 70 }]);
        setTimeout(() => {
  ...
  const sendReaction = (emoji) => {
    const newRef = push(reactionsRef(matchId));
    set(newRef, { emoji, at: Date.now() });
    // Self-cleanup: remove this reaction from RTDB a few seconds later so the list doesn't grow forever
    setTimeout(() => remove(newRef).catch(() => {}), 4000);
  };

  if (showTiebreakerIntro) {
    return (
      <div style={{ position: "fixed", inset: 0, background: "radial-gradient(circle at center, #2a0a0a 0%, #0a0a0f 70%)", zIndex: 10000, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 24, textAlign: "center" }}>
        <div style={{ fontSize: 52, marginBottom: 8, animation: "pulse 1.5s infinite" }}>⚔️</div>
        <div style={{ fontSize: 12, fontWeight: 800, color: "#ef4444", letterSpacing: 2, textTransform: "uppercase", marginBottom: 16 }}>Tiebreaker Decider</div>
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <Avatar name={match.whiteName} size={64} photo={getPlayerPhoto(match.whiteId)} />
            <span style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>{match.whiteName}</span>
            <span style={{ color: "#9ca3af", fontSize: 11 }}>⬜ White</span>
          </div>
          <div style={{ fontSize: 28, fontWeight: 900, color: "#ef4444" }}>VS</div>
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <Avatar name={match.blackName} size={64} photo={getPlayerPhoto(match.blackId)} />
            <span style={{ color: "#fff", fontWeight: 800, fontSize: 16 }}>{match.blackName}</span>
            <span style={{ color: "#9ca3af", fontSize: 11 }}>⬛ Black</span>
          </div>
        </div>
        <div style={{ fontSize: 13, color: "#d1d5db", maxWidth: 320, lineHeight: 1.6, marginBottom: 24 }}>
          These two players are tied in every way the standings can measure — points, head-to-head, even who won as White. This match alone decides who ranks higher.
        </div>
        <button
          onClick={() => setShowTiebreakerIntro(false)}
          style={{ background: "#ef4444", color: "#fff", border: "none", borderRadius: 10, padding: "14px 32px", fontWeight: 800, fontSize: 15, cursor: "pointer" }}
        >
          ⚔️ Enter the Decider
        </button>
      </div>
  ...
  // A player resigns — the other side wins immediately, regardless of board position.
  const resign = () => {
    if (myColor !== "white" && myColor !== "black") return;
    if (game.status !== "playing") return;
    const winnerColor = myColor === "white" ? "black" : "white";
    set(liveGameRef(matchId), {
      ...game,
      status: "ended",
      resignedBy: myColor,
      updatedAt: Date.now(),
  ...
  // Offers a draw to the opponent. Stored on the live game record so both sides see the
  // pending offer in real time, regardless of whose device made it.
  const offerDraw = () => {
    if (myColor !== "white" && myColor !== "black") return;
    if (game.status !== "playing") return;
    set(liveGameRef(matchId), { ...game, drawOfferBy: myColor, updatedAt: Date.now() });
  };

  const acceptDraw = () => {
    if (game.status !== "playing" || !game.drawOfferBy) return;
    set(liveGameRef(matchId), { ...game, status: "ended", drawOfferBy: null, drawAccepted: true, updatedAt: Date.now() });
    onGameEnd && onGameEnd(matchId, "draw");
  };

  const declineDraw = () => {
    if (!game.drawOfferBy) return;
    set(liveGameRef(matchId), { ...game, drawOfferBy: null, updatedAt: Date.now() });
  };
  ...
  return (
    ...
    >
      <div style={{ width: "100%", maxWidth: 480, userSelect: "none", WebkitUserSelect: "none" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
          <div style={{ fontWeight: 800, color: "#fff", fontSize: 14, display: "flex", alignItems: "center" }}>
            ...
            {match.isTiebreaker ? "⚔️ Tiebreaker Decider" : "Live Match"}
          </div>
        ...
        {game.status === "ended" && (
          <div style={{ textAlign: "center", color: "#f0c040", fontWeight: 800, fontSize: 15, margin: "10px 0", padding: 12, background: "rgba(240,192,64,0.1)", borderRadius: 8 }}>
            {game.isCheckmate
              ? `♟️ Checkmate! ${chess.turn() === "w" ? match.blackName : match.whiteName} wins!`
              : game.resignedBy
              ? `🏳️ ${game.resignedBy === "white" ? match.whiteName : match.blackName} resigned — ${game.resignedBy === "white" ? match.blackName : match.whiteName} wins!`
              : game.drawAccepted
              ? "🤝 Draw agreed!"
              : "Game over!"}
          </div>
        ...
        {/* Pending draw offer banner — shown to whoever didn't make the offer, so they can respond */}
        {game.status === "playing" && game.drawOfferBy && game.drawOfferBy !== myColor && (myColor === "white" || myColor === "black") && (
          <div style={{ textAlign: "center", margin: "8px 0", padding: 12, background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.3)", borderRadius: 8 }}>
            <div style={{ color: "#93c5fd", fontWeight: 700, fontSize: 13, marginBottom: 8 }}>
              🤝 {game.drawOfferBy === "white" ? match.whiteName : match.blackName} offers a draw
            </div>
            <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
              <button onClick={acceptDraw} style={{ background: "#22c55e", color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>✅ Accept</button>
              <button onClick={declineDraw} style={{ background: "#374151", color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>❌ Decline</button>
            </div>
          </div>
        )}
        {game.status === "playing" && game.drawOfferBy && game.drawOfferBy === myColor && (
          <div style={{ textAlign: "center", margin: "8px 0", fontSize: 12, color: "#93c5fd" }}>
            🤝 Draw offer sent — waiting for {myColor === "white" ? match.blackName : match.whiteName} to respond...
          </div>
        )}
        ...
        {/* Resign / Offer Draw — only for the two active players, only while the game is live */}
        {game.status === "playing" && (myColor === "white" || myColor === "black") && (
          <div style={{ display: "flex", gap: 8, margin: "8px 0" }}>
            <button
              onClick={() => {
                if (window.confirm("Are you sure you want to resign? Your opponent will win immediately.")) resign();
              }}
              style={{ flex: 1, background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", color: "#ef4444", borderRadius: 8, padding: "10px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}
            >
              🏳️ Resign
            </button>
            {!game.drawOfferBy && (
              <button
                onClick={offerDraw}
                style={{ flex: 1, background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.3)", color: "#93c5fd", borderRadius: 8, padding: "10px", fontWeight: 700, fontSize: 12, cursor: "pointer" }}
              >
                🤝 Offer Draw
              </button>
            )}
        ...
        {/* Reaction picker — available to everyone watching, players and spectators alike */}
        <div style={{ display: "flex", justifyContent: "center", gap: 10, marginTop: 10 }}>
          {["🔥", "👏", "😱", "♥️", "😂"].map(emoji => (
            <button
              key={emoji}
              onClick={() => sendReaction(emoji)}
              style={{ background: "#1a1f2e", border: `1px solid #2a2f3e`, borderRadius: "50%", width: 44, height: 44, fontSize: 20, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}
            >
              {emoji}
            </button>
          ))}
      ...
      {/* Floating reactions — positioned over the whole overlay, animate up and fade out */}
...
export default function ChessClub() {
  ...
  // Tiebreaker decider matches: [{ id, playerAId, playerBId, status: "pending_approval"|"approved"|"resolved", matchId }]
  const [tiebreakers, setTiebreakers] = useState([]);
  // Player of the Month: potmAwards = approved history [{ id, playerId, playerName, month, year, winRate, wins, games }]
  // pendingPotm = the auto-calculated candidate for last month, awaiting Head Admin approval (or null)
  const [potmAwards, setPotmAwards] = useState([]);
  const [pendingPotm, setPendingPotm] = useState(null);
    ...
    const unsubscribe = onSnapshot(
      ...
      (snap) => {
        if (snap.exists()) {
          ...
          setPotmAwards(data.potmAwards || []);
          setPendingPotm(data.pendingPotm || null);
        }
        setDbLoaded(true);
        setDbError(null);
      },
  ...
  useEffect(() => {
    ...
    setDoc(CLUB_DOC, { players, matches, results, activity, seasons, currentSeasonName, announcements, tiebreakers, potmAwards, pendingPotm, updatedAt: Date.now() }).catch((err) => {
      console.error("Firestore save error:", err);
      setDbError(err.message);
    });
  }, [players, matches, results, activity, seasons, currentSeasonName, announcements, tiebreakers, potmAwards, pendingPotm, dbLoaded]);
  ...
  const standings = useMemo(() => {
    ...
    const playerList = Object.values(map).map(p => ({ ...p, mp: p.w + p.d + p.l }));

    // Head-to-head lookup: for any two player IDs, returns their direct results against each other.
    const getH2HResults = (idA, idB) => {
      return results
        .map(r => ({ r, match: matches.find(m => m.id === r.matchId) }))
        .filter(({ match }) => match && ((match.whiteId === idA && match.blackId === idB) || (match.whiteId === idB && match.blackId === idA)))
        .map(({ r, match }) => ({
          winnerId: r.winner === "white" ? match.whiteId : r.winner === "black" ? match.blackId : null,
          winnerPlayedWhite: r.winner !== "draw",
          whiteId: match.whiteId,
        }));
    };

    // Compares two tied players using the tiebreaker chain. Returns negative if A ranks above B,
    // positive if B ranks above A, or 0 if fully unresolved (needs a decider match).
    const compareTiedPair = (a, b) => {
      // If a tiebreaker decider match exists for this exact pair and has been played, its
      // result directly decides rank order — overriding the symmetric tiebreaker chain below.
      const tb = (tiebreakers || []).find(t =>
        t.status === "approved" && t.matchId &&
        ((t.playerAId === a.id && t.playerBId === b.id) || (t.playerAId === b.id && t.playerBId === a.id))
      );
      if (tb) {
        const tbResult = results.find(r => r.matchId === tb.matchId);
        if (tbResult && tbResult.winner !== "draw") {
          const tbMatch = matches.find(m => m.id === tb.matchId);
          if (tbMatch) {
            const winnerId = tbResult.winner === "white" ? tbMatch.whiteId : tbMatch.blackId;
      ...
      const h2h = getH2HResults(a.id, b.id);
      let aWins = 0, bWins = 0;
      let aWonAsWhite = false, bWonAsWhite = false;
      h2h.forEach(({ winnerId, whiteId }) => {
        if (winnerId === a.id) { aWins++; if (whiteId === a.id) aWonAsWhite = true; }
        else if (winnerId === b.id) { bWins++; if (whiteId === b.id) bWonAsWhite = true; }
      });
      if (aWins !== bWins) return bWins - aWins; // more head-to-head wins ranks higher
      // Head-to-head record is even (e.g. 1-1, or both draws). Check "home" (White) win.
      if (aWonAsWhite && !bWonAsWhite) return -1;
      if (bWonAsWhite && !aWonAsWhite) return 1;
      return 0; // fully symmetric — both won as White, or neither won at all. Needs a decider match.
    };

    const sorted = [...playerList].sort((a, b) => {
      if (b.pts !== a.pts) return b.pts - a.pts;
      return compareTiedPair(a, b);
    });

    return sorted;
  }, [players, matches, results, tiebreakers]);

  // Detects pairs of adjacent, equal-points players whose tiebreak is fully unresolved (both
  // won their head-to-head match as White) — these need a manually-approved decider match.
  const unresolvedTiebreakPairs = useMemo(() => {
    const pairs = [];
    for (let i = 0; i < standings.length - 1; i++) {
      ...
      const h2h = results
        .map(r => ({ r, match: matches.find(m => m.id === r.matchId) }))
        .filter(({ match }) => match && ((match.whiteId === a.id && match.blackId === b.id) || (match.whiteId === b.id && match.blackId === a.id)));
      if (h2h.length < 2) continue; // haven't played both legs yet, nothing to resolve
      let aWins = 0, bWins = 0, aWonAsWhite = false, bWonAsWhite = false;
      h2h.forEach(({ r, match }) => {
        const winnerId = r.winner === "white" ? match.whiteId : r.winner === "black" ? match.blackId : null;
        if (winnerId === a.id) { aWins++; if (match.whiteId === a.id) aWonAsWhite = true; }
        else if (winnerId === b.id) { bWins++; if (match.whiteId === b.id) bWonAsWhite = true; }
      });
      const stillTied = aWins === bWins && ((aWonAsWhite && bWonAsWhite) || (!aWonAsWhite && !bWonAsWhite && aWins > 0));
      if (stillTied) {
        // Skip if a tiebreaker for this exact pair was already approved/resolved (it's handled).
        // Dismissed suggestions are allowed to resurface so the Head Admin sees the reminder again.
        const alreadyHandled = (tiebreakers || []).some(tb =>
          (tb.status === "approved" || tb.status === "resolved") &&
          ((tb.playerAId === a.id && tb.playerBId === b.id) || (tb.playerAId === b.id && tb.playerBId === a.id))
        );
        if (!alreadyHandled) pairs.push({ playerAId: a.id, playerBId: b.id, playerAName: a.name, playerBName: b.name, rank: i + 1 });
      }
    }
    return pairs;
  }, [standings, results, matches, tiebreakers]);

  const { downloadAndShare } = useStandingsImage(standings, currentSeasonName);

  // Calculates the best-win-rate player for a given month/year (0-indexed month, like JS Date).
  // Only counts matches whose date falls within that calendar month, and requires at least 5
  // games played that month to qualify — otherwise a single lucky win could "win" the award.
  const calculatePotmForMonth = (year, month) => {
    const monthStr = String(month + 1).padStart(2, "0");
    const prefix = `${year}-${monthStr}`;
    const monthResults = results.filter(r => {
      const match = matches.find(m => m.id === r.matchId);
      return match && match.date && match.date.startsWith(prefix);
    });
    if (monthResults.length === 0) return null;

    const tally = {}; // playerId -> { wins, games }
    players.forEach(p => { tally[p.id] = { wins: 0, games: 0 }; });
    monthResults.forEach(r => {
      const match = matches.find(m => m.id === r.matchId);
      if (!match) return;
      const wId = match.whiteId, bId = match.blackId;
      if (!tally[wId] || !tally[bId]) return;
      tally[wId].games++; tally[bId].games++;
      if (r.winner === "white") tally[wId].wins++;
      else if (r.winner === "black") tally[bId].wins++;
    });

    let best = null;
    Object.entries(tally).forEach(([playerId, stat]) => {
      if (stat.games < 5) return; // minimum games threshold
      const winRate = stat.wins / stat.games;
      if (!best || winRate > best.winRate) {
        const player = players.find(p => p.id === +playerId);
        if (!player) return;
        best = { playerId: +playerId, playerName: player.name, winRate, wins: stat.wins, games: stat.games };
      }
    });
    return best;
  };

  // On load (and whenever the month changes while the app stays open), check if last month's
  // award has already been calculated/approved. If not, auto-calculate a candidate and surface
  // it for Head Admin approval — never auto-publishes without a tap.
  useEffect(() => {
    if (!dbLoaded) return;
    const now = new Date();
    ...
    const monthLabel = lastMonthDate.toLocaleString("default", { month: "long", year: "numeric" });

    const alreadyAwarded = potmAwards.some(a => a.year === y && a.month === m);
    const alreadyPending = pendingPotm && pendingPotm.year === y && pendingPotm.month === m;
    if (alreadyAwarded || alreadyPending) return;

    const candidate = calculatePotmForMonth(y, m);
    if (candidate) {
      setPendingPotm({ ...candidate, year: y, month: m, monthLabel });
    }
  }, [dbLoaded, players, matches, results]);

  const approvePotm = () => {
    if (!pendingPotm) return;
    setPotmAwards(prev => [...prev, { id: Date.now(), ...pendingPotm }]);
    setPendingPotm(null);
    showNotif("info", "🏅 Player of the Month!", `${pendingPotm.playerName} is now Player of the Month for ${pendingPotm.monthLabel}!`);
    logActivity("🏅", `${pendingPotm.playerName} awarded Player of the Month for ${pendingPotm.monthLabel} (${pendingPotm.wins}/${pendingPotm.games} wins, ${(pendingPotm.winRate * 100).toFixed(0)}% win rate).`);
  };
  ...
  const logActivity = (icon, text) => {
    const actor = isAdmin ? "Head Admin" : isMod ? `${currentUser?.name || "Moderator"} (Mod)` : (currentUser?.name || "Someone");
    setActivity(prev => [...prev, { id: Date.now(), icon, text, ts: Date.now(), actor }].slice(-50));
  };
  ...
  // Checks every 30 seconds whether any upcoming match's scheduled date+time has arrived, and
  // if so, starts it automatically (same as tapping "Start" manually) — no one needs to be
  // watching the clock. Only runs for people with permission to start matches, so it doesn't
  // fire redundantly from every spectator's browser tab.
  useEffect(() => {
    if (!dbLoaded || !can("startMatches")) return;
    const checkAndAutoStart = () => {
      const now = new Date();
      matches.forEach(m => {
        if (m.started) return;
        if (!m.date || !m.time) return; // only auto-start matches that actually have a scheduled time
        const scheduled = new Date(`${m.date}T${m.time}:00`);
        if (now >= scheduled) {
          startMatch(m.id);
        }
      });
    };
    checkAndAutoStart(); // check immediately on load too, in case the app was closed past the start time
    const interval = setInterval(checkAndAutoStart, 30000); // re-check every 30 seconds
    return () => clearInterval(interval);
  }, [matches, dbLoaded, currentUser]);
  ...
  const recordResult = (matchId, winner) => {
    ...
    // If this match was a tiebreaker decider, mark it resolved and announce the winner specially.
    const tb = tiebreakers.find(t => t.matchId === matchId && t.status === "approved");
    if (tb) {
      setTiebreakers(prev => prev.map(t => t.id === tb.id ? { ...t, status: "resolved" } : t));
      if (winnerName) {
        showNotif("end", "⚔️ Tiebreaker Decided!", `${winnerName} wins the decider and takes the higher rank!`);
        logActivity("⚔️", `Tiebreaker decided: ${winnerName} defeated ${winnerName === match.whiteName ? match.blackName : match.whiteName} to claim the higher rank.`);
      } else {
        showNotif("end", "⚔️ Tiebreaker Drawn!", `${match.whiteName} vs ${match.blackName} ended in a draw — rank remains unresolved, a rematch may be needed.`);
        logActivity("⚔️", `Tiebreaker decider ended in a draw: ${match.whiteName} vs ${match.blackName}.`);
      }
    } else {
  ...
  // Builds a full double round-robin match list (with rounds and dates) from the current player
  // list, and applies it via setMatches. Shared by endSeason() and the manual "Generate Schedule"
  // button (for the very first season, before any season has ended yet).
  const buildRoundRobinSchedule = () => {
    if (players.length % 2 !== 0) {
      showNotif("info", "⚠️ Even Number Required", `You have ${players.length} players. The round-robin schedule needs an EVEN number of players. Add or remove one player first.`);
      return null;
    }
    const pairingRounds = generateRoundRobinPairings(players);
    const roundDates = getRoundDates(pairingRounds.length);
    const timeSlots = assignTimeSlots(pairingRounds);
    const TIMES = ["21:00", "22:00"]; // 9 PM, 10 PM
    let matchIdCounter = Date.now();
    const newMatches = [];
    pairingRounds.forEach((roundPairs, roundIdx) => {
      const dateStr = formatDateYMD(roundDates[roundIdx]);
      roundPairs.forEach(([white, black], matchIdx) => {
        const slot = timeSlots[roundIdx][matchIdx];
        newMatches.push({
          id: matchIdCounter++,
          whiteId: white.id,
          blackId: black.id,
  ...
  // Manual trigger for generating the round-robin schedule for the CURRENT season — mainly for
  // the very first season, since endSeason() only runs when transitioning between seasons.
  const generateScheduleForCurrentSeason = () => {
    const newMatches = buildRoundRobinSchedule();
    if (!newMatches) return;
    setMatches(newMatches);
    showNotif("info", "📅 Schedule Generated!", `${currentSeasonName} now has a ${newMatches.length}-match round-robin schedule (${newMatches[newMatches.length - 1]?.round || 0} rounds).`);
    logActivity("📅", `Round-robin schedule generated for ${currentSeasonName} (${newMatches.length} matches).`);
  };

  // === TIEBREAKER DECIDER MATCHES ===
  // When two players are fully tied even after head-to-head and "home" (White) win checks,
  // the app detects it (see unresolvedTiebreakPairs) and asks the Head Admin to approve a
  // special decider match between them before it's created.
  const approveTiebreaker = (pair) => {
    const playerA = players.find(p => p.id === pair.playerAId);
    const playerB = players.find(p => p.id === pair.playerBId);
    if (!playerA || !playerB) return;
    const tiebreakerId = Date.now();
    const matchId = tiebreakerId + 1;
    const newMatch = {
      id: matchId,
      whiteId: playerA.id,
      blackId: playerB.id,
      whiteName: playerA.name,
      blackName: playerB.name,
    ...
    showNotif("info", "⚔️ Tiebreaker Approved!", `${playerA.name} vs ${playerB.name} will decide who ranks higher in the table.`);
    logActivity("⚔️", `Tiebreaker decider match approved: ${playerA.name} vs ${playerB.name}`);
  };

  // If the Head Admin doesn't want to schedule a decider right now, they can dismiss the
  // suggestion — it'll reappear next time standings are calculated unless re-dismissed.
  const dismissTiebreakerSuggestion = (pair) => {
    setTiebreakers(prev => [...prev, {
      id: Date.now(),
  ...
  const endSeason = () => {
    ...
    if (players.length % 2 !== 0) {
      showNotif("info", "⚠️ Even Number Required", `You have ${players.length} players. The round-robin schedule needs an EVEN number of players to start a new season. Add or remove one player first.`);
      return;
    }
    ...
    // Auto-generate the new season's full double round-robin schedule (everyone plays everyone
    // twice, colors swapped on the rematch), spaced two rounds per week (Saturday & Tuesday).
    const newMatches = buildRoundRobinSchedule();
    ...
    const roundCount = newMatches && newMatches.length > 0 ? newMatches[newMatches.length - 1].round : 0;
    showNotif("end", "🏆 Season Ended!", `${champion.name} won ${seasonRecord.name} with ${champion.pts} pts! ${newName} has begun with a ${roundCount}-round schedule.`);
    logActivity("🏆", `${seasonRecord.name} ended — Champion: ${champion.name} (${champion.pts} pts). ${newName} begins with auto-generated round-robin schedule (${roundCount} rounds).`);
  };
  ...
  return (
    <div style={s.app}>
      <style>{`@keyframes slideIn{from{opacity:0;transform:translateY(-16px)}to{opacity:1;transform:translateY(0)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.3}} @keyframes floatUp{0%{opacity:0;transform:translateY(0) scale(0.5)}15%{opacity:1;transform:translateY(-20px) scale(1.1)}85%{opacity:1}100%{opacity:0;transform:translateY(-220px) scale(1)}}
        ...
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 10000, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => { setSelectedPlayer(null); setEditingPlayer(false); }}>
            <div style={{ background: T.bgCard, border: `1px solid ${T.border}`, borderRadius: "16px 16px 0 0", padding: 24, width: "100%", maxWidth: 640, maxHeight: "85vh", overflowY: "auto" }} onClick={e => e.stopPropagation()}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 16 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 4 }}>
                      ...
                      {potmAwards.filter(a => a.playerId === player.id).length > 0 && (
                        <Badge text={`🏅 POTM x${potmAwards.filter(a => a.playerId === player.id).length}`} color="#92400e" />
                      )}
                    </div>
                  </div>
                  {can("addPlayers") && (
              ...
              {/* Player of the Month awards for this player */}
              {potmAwards.filter(a => a.playerId === player.id).length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#6b7280", letterSpacing: 1, textTransform: "uppercase", marginBottom: 8 }}>🏅 Player of the Month</div>
                  {potmAwards.filter(a => a.playerId === player.id).slice().reverse().map(award => (
                    <div key={award.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 0", borderBottom: "1px solid #2a2f3e" }}>
                      <div style={{ fontSize: 13, color: T.textBright, fontWeight: 600 }}>{award.monthLabel}</div>
                      <div style={{ fontSize: 12, color: "#f0c040", fontWeight: 700 }}>{award.wins}W / {award.games} games · {(award.winRate * 100).toFixed(0)}%</div>
                    </div>
                  ))}
                </div>
              )}
      ...
      <div style={s.header}>
        <div style={s.topRow}>
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            <div style={s.roleBadge}>{isAdmin ? "👑 Head Admin" : isMod ? "🛡️ Moderator" : currentUser?.playerId ? "♟️ Viewer" : "👤 Visitor"}</div>
      ...
      <div style={s.body}>
        ...
        {tab === "Dashboard" && (
          <>
            ...
            {/* Personal stats — only shown to linked players (those who claimed a profile via invite code) */}
            {currentUser?.playerId && (() => {
              const myPlayer = players.find(p => p.id === currentUser.playerId);
              if (!myPlayer) return null;
              const myStat = standings.find(s2 => s2.id === currentUser.playerId) || { w: 0, d: 0, l: 0, pts: 0, mp: 0, elo: DEFAULT_ELO };
              const myForm = getForm(currentUser.playerId, 5);
              const myNextMatch = notStartedMatches.find(m => m.whiteId === currentUser.playerId || m.blackId === currentUser.playerId);
              const myLiveMatch = startedMatches.find(m => m.whiteId === currentUser.playerId || m.blackId === currentUser.playerId);
              const myPotmCount = potmAwards.filter(a => a.playerId === currentUser.playerId).length;
              const myRank = standings.findIndex(s2 => s2.id === currentUser.playerId) + 1;

              return (
                <div style={{ ...s.card, border: "1px solid rgba(59,130,246,0.3)", background: "linear-gradient(135deg, rgba(59,130,246,0.08), rgba(59,130,246,0.02))", marginBottom: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                    <Avatar name={myPlayer.name} size={48} photo={myPlayer.photo} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 800, fontSize: 17, color: T.textBright }}>
                        {myPlayer.name}
                        {myPotmCount > 0 && <span style={{ fontSize: 12, color: "#f0c040", fontWeight: 700, marginLeft: 8 }}>🏅×{myPotmCount}</span>}
                      </div>
                      <div style={{ fontSize: 12, color: T.textMuted2 }}>
                        {myRank > 0 ? `Rank #${myRank}` : "Unranked"} · ELO {myStat.elo || DEFAULT_ELO}
                      </div>
                    </div>
                    <button onClick={() => { setSelectedPlayer(currentUser.playerId); setEditingPlayer(false); }}
                      style={{ background: "rgba(59,130,246,0.12)", border: "1px solid rgba(59,130,246,0.3)", color: "#93c5fd", borderRadius: 8, padding: "6px 12px", fontWeight: 700, fontSize: 11, cursor: "pointer" }}>
                      View Profile
                    </button>
                  </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 12 }}>
                    <div style={{ background: T.bg, borderRadius: 8, padding: "8px 4px", textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: "#f0c040" }}>{myStat.pts}</div>
                      <div style={{ fontSize: 9, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.4 }}>Pts</div>
                    </div>
                    <div style={{ background: T.bg, borderRadius: 8, padding: "8px 4px", textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: "#22c55e" }}>{myStat.w}</div>
                      <div style={{ fontSize: 9, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.4 }}>Wins</div>
                    </div>
                    <div style={{ background: T.bg, borderRadius: 8, padding: "8px 4px", textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: "#9ca3af" }}>{myStat.d}</div>
                      <div style={{ fontSize: 9, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.4 }}>Draws</div>
                    </div>
                    <div style={{ background: T.bg, borderRadius: 8, padding: "8px 4px", textAlign: "center" }}>
                      <div style={{ fontSize: 18, fontWeight: 800, color: "#ef4444" }}>{myStat.l}</div>
                      <div style={{ fontSize: 9, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.4 }}>Losses</div>
                    </div>
                  </div>
                  ...
                  {myForm.length > 0 && (
                    <div style={{ marginBottom: 12 }}>
                      <div style={{ fontSize: 10, color: T.textMuted, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 6 }}>Recent Form</div>
                      <FormBadges form={myForm} size={22} />
                    </div>
                  )}

                  {/* Live match */}
                  {myLiveMatch && (
                    <div style={{ background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.25)", borderRadius: 8, padding: "10px 12px", marginBottom: 8, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      <div>
                        <div style={{ fontSize: 11, fontWeight: 700, color: "#22c55e" }}>🟢 Your match is LIVE now!</div>
                        <div style={{ fontSize: 12, color: T.textMuted2 }}>
                          vs {myLiveMatch.whiteId === currentUser.playerId ? myLiveMatch.blackName : myLiveMatch.whiteName}
                        </div>
                      </div>
                      <button style={{ ...s.startBtn, background: "#3b82f6" }} onClick={() => openLiveBoard(myLiveMatch)}>♟️ Join</button>
                    </div>
                  )}

                  {/* Next match */}
                  {myNextMatch && !myLiveMatch && (
                    <div style={{ background: T.bg, borderRadius: 8, padding: "10px 12px", fontSize: 12, color: T.textMuted2 }}>
                      <span style={{ color: T.textBright, fontWeight: 600 }}>📅 Next:</span> vs {myNextMatch.whiteId === currentUser.playerId ? myNextMatch.blackName : myNextMatch.whiteName} · {myNextMatch.date}{myNextMatch.time ? ` @ ${myNextMatch.time}` : ""}
                    </div>
                  )}
                </div>
              );
            ...
            {announcements.length > 0 && announcements.slice(0, 3).map(a => (
              <div key={a.id} style={{ ...s.card, background: "rgba(59,130,246,0.07)", border: "1px solid rgba(59,130,246,0.25)", padding: "14px 16px", marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8 }}>
                  ...
                  {can("deleteAnnouncements") && <button onClick={() => deleteAnnouncement(a.id)} style={{ background: "none", border: "none", color: "#ef4444", cursor: "pointer", fontSize: 16, flexShrink: 0 }}>×</button>}
                </div>
              </div>
            ))}

            {/* Player of the Month — pending approval, Head Admin only */}
            {isAdmin && pendingPotm && (
              <div style={{ ...s.card, border: "1px solid rgba(240,192,64,0.4)", background: "linear-gradient(135deg, rgba(240,192,64,0.12), rgba(240,192,64,0.03))" }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: "#f0c040", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>
                  🏅 Player of the Month — Pending Approval
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
                  <Avatar name={pendingPotm.playerName} size={48} photo={getPlayerPhoto(pendingPotm.playerId)} />
                  <div>
                    <div style={{ fontWeight: 800, fontSize: 16, color: T.textBright }}>{pendingPotm.playerName}</div>
                    <div style={{ fontSize: 12, color: T.textMuted2 }}>{pendingPotm.monthLabel} — {pendingPotm.wins}/{pendingPotm.games} wins ({(pendingPotm.winRate * 100).toFixed(0)}% win rate)</div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button style={{ ...s.btn(), marginTop: 0, flex: 1 }} onClick={approvePotm}>🏅 Approve & Announce</button>
                  <button style={{ ...s.btn("#374151"), marginTop: 0, flex: 1 }} onClick={dismissPotm}>Dismiss</button>
                </div>
              </div>
            )}

            {/* Player of the Month — featured display, everyone sees once approved */}
            {potmAwards.length > 0 && (() => {
              const latest = potmAwards[potmAwards.length - 1];
              return (
                <div style={{ ...s.card, border: "1px solid rgba(240,192,64,0.3)", background: "linear-gradient(135deg, rgba(240,192,64,0.1), rgba(240,192,64,0.02))", textAlign: "center" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#f0c040", letterSpacing: 1, textTransform: "uppercase", marginBottom: 10 }}>🏅 Player of the Month</div>
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, cursor: "pointer" }} onClick={() => setSelectedPlayer(latest.playerId)}>
                    <Avatar name={latest.playerName} size={56} photo={getPlayerPhoto(latest.playerId)} />
                    <div style={{ fontSize: 18, fontWeight: 800, color: T.textBright }}>{latest.playerName}</div>
                    <div style={{ fontSize: 12, color: T.textMuted2 }}>{latest.monthLabel} — {(latest.winRate * 100).toFixed(0)}% win rate</div>
                  </div>
                </div>
              );
            })()}
                  ...
                  <div key={a.id} style={{ display: "flex", alignItems: "flex-start", gap: 10, padding: "8px 0", borderBottom: `1px solid ${T.border}` }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 10, color: T.textMuted, marginTop: 2 }}>
                        {new Date(a.ts).toLocaleString()}{a.actor ? ` · by ${a.actor}` : ""}
                      </div>
                    </div>
                  </div>
                ))}
        ...
        {tab === "Schedule" && (
          <>
            {can("scheduleMatches") && matches.length === 0 && (
              <div style={{ ...s.card, border: "1px solid rgba(124,58,237,0.3)" }}>
                <div style={s.sectionTitle}>🔄 Round-Robin Schedule</div>
                <div style={{ fontSize: 12, color: T.textMuted2, marginBottom: 12, lineHeight: 1.5 }}>
                  Auto-generate the full {currentSeasonName} schedule — every player faces every other player twice (colors swap on the rematch), 2 rounds per week on Saturday & Tuesday. Requires an even number of players.
                </div>
                <button
                  style={{ ...s.btn("#7c3aed") }}
                  disabled={players.length < 2}
                  onClick={generateScheduleForCurrentSeason}
                >
                  📅 Generate Round-Robin Schedule ({players.length} players)
                </button>
              </div>
            )}
            ...
            {/* === ROUND VIEW (primary) === */}
            {(() => {
              const allRoundNumbers = Array.from(new Set(matches.filter(m => m.round).map(m => m.round))).sort((a, b) => a - b);
              const noRoundMatches = matches.filter(m => !m.round);

              if (allRoundNumbers.length === 0) {
                // No round-robin schedule yet — nothing to show here, manual matches appear lower down.
                return null;
              }

              const effectiveRound = allRoundNumbers.includes(roundView) ? roundView : allRoundNumbers[0];
              ...
              const RoundMatchCard = (m) => {
                const status = getMatchStatus(m);
                const result = results.find(r => r.matchId === m.id);
                const winnerName = result ? (result.winner === "white" ? m.whiteName : result.winner === "black" ? m.blackName : null) : null;
                return (
                  <div key={m.id} style={{ border: `1px solid ${T.border}`, borderRadius: 10, padding: 12, marginBottom: 10 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <span style={{ fontWeight: 600, fontSize: 13, color: T.textBright }}>{m.whiteName}</span>
                      <span style={{ color: status === "live" ? "#22c55e" : "#f0c040", fontWeight: 800, fontSize: 12 }}>VS</span>
                      <Avatar name={m.blackName} size={30} photo={getPlayerPhoto(m.blackId)} />
                      <span style={{ fontWeight: 600, fontSize: 13, color: T.textBright }}>{m.blackName}</span>
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                      {status === "upcoming" && <Badge text={m.date} color="#1d3557" />}
                      {status === "live" && <Badge text="🟢 LIVE" color="#15532e" />}
                      {status === "finished" && <Badge text={!winnerName ? `🤝 Draw` : `🏆 ${winnerName}`} color={!winnerName ? "#374151" : "#b45309"} />}

                      <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
                        {status === "live" && (
                          <button style={{ ...s.startBtn, background: "#3b82f6" }} onClick={() => openLiveBoard(m)}>♟️ Watch</button>
                        )}
                        {status === "upcoming" && can("startMatches") && (
                          <button style={s.startBtn} onClick={() => startMatch(m.id)}>▶ Start</button>
                        )}
                        {status === "upcoming" && can("cancelMatches") && (
                          <button
                            style={{ ...s.startBtn, background: "#ef4444" }}
                            onClick={() => {
                              if (window.confirm(`Cancel match: ${m.whiteName} vs ${m.blackName}?`)) cancelMatch(m.id);
                            }}
                          >✕ Cancel</button>
                        )}
                      </div>
                    </div>
                    {status === "upcoming" && m.time && <div style={{ fontSize: 12, color: "#6b7280", marginTop: 4 }}>🕐 {m.time}</div>}
                  </div>
                );
              };
              ...
              return (
                <div style={s.card}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                    <div style={s.sectionTitle}>🔄 Round {effectiveRound}</div>
                    <select
                      style={{ ...s.select, width: "auto", padding: "8px 10px" }}
                      value={effectiveRound}
                      onChange={e => setRoundView(Number(e.target.value))}
                    >
                      {allRoundNumbers.map(rn => <option key={rn} value={rn}>Round {rn}</option>)}
                    </select>
                  </div>
                  {roundMatches.map(RoundMatchCard)}
                </div>
              );
            })()}

            {/* === MANUAL OVERRIDE (collapsed) === */}
            {can("scheduleMatches") && !showManualSchedule && (
              <button
              ...
              >
                ⚙️ Manual Override (schedule a one-off match outside the round-robin)
              </button>
            )}
            {can("scheduleMatches") && showManualSchedule && (
              <div style={s.card}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                  ...
                  <button onClick={() => setShowManualSchedule(false)} style={{ background: "none", border: "none", color: T.textMuted, fontSize: 20, cursor: "pointer", lineHeight: 1 }}>×</button>
                </div>
                {players.length < 2 ? <div style={s.empty}>Add at least 2 players first.</div> : (
                  <>
                    ...
                    <button style={s.btn()} onClick={() => { addMatch(); setShowManualSchedule(false); }}>📅 Schedule & Notify Group</button>
                  </>
                )}
              </div>
            )}

            {/* === Tiebreaker decider matches — special styling, shown separately === */}
            {(() => {
              const tbMatches = matches.filter(m => m.isTiebreaker && !results.find(r => r.matchId === m.id));
              if (tbMatches.length === 0) return null;
              return (
                <div style={{ ...s.card, border: "2px solid #ef4444", background: "linear-gradient(135deg, rgba(239,68,68,0.1), rgba(124,58,237,0.06))" }}>
                  <div style={s.sectionTitle}>⚔️ Tiebreaker {tbMatches.length > 1 ? "Matches" : "Match"}</div>
                  {tbMatches.map(m => (
                    <div key={m.id} style={{ padding: "12px 0", borderBottom: "1px solid rgba(239,68,68,0.2)" }}>
                      <div style={{ textAlign: "center", marginBottom: 8 }}>
                        <Badge text="⚔️ DECIDES RANK ORDER" color="#7c1d1d" />
                      </div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", justifyContent: "center" }}>
                        <Avatar name={m.whiteName} size={34} photo={getPlayerPhoto(m.whiteId)} />
                        <span style={{ fontWeight: 700, fontSize: 14, color: T.textBright }}>{m.whiteName}</span>
                        <span style={{ color: "#ef4444", fontWeight: 900, fontSize: 13 }}>⚔️ VS</span>
                        <Avatar name={m.blackName} size={34} photo={getPlayerPhoto(m.blackId)} />
                        <span style={{ fontWeight: 700, fontSize: 14, color: T.textBright }}>{m.blackName}</span>
                      </div>
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 8, marginTop: 10, flexWrap: "wrap" }}>
                        {m.started ? (
                          ...
                          <>
                            {can("startMatches") && <button style={s.startBtn} onClick={() => startMatch(m.id)}>▶ Start Decider</button>}
                          </>
                        )}
                      </div>
                    </div>
            ...
            {/* === Matches outside the round-robin (manually scheduled) === */}
            {(() => {
              const noRoundMatches = matches.filter(m => !m.round && !m.isTiebreaker && !results.find(r => r.matchId === m.id));
              const noRoundLive = noRoundMatches.filter(m => m.started);
              const noRoundUpcoming = noRoundMatches.filter(m => !m.started);
              if (noRoundMatches.length === 0) return null;
              return (
                <div style={s.card}>
                  <div style={s.sectionTitle}>Other Matches ({noRoundMatches.length})</div>
                  {noRoundLive.map(m => (
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                        ...
                        <div style={{ display: "flex", gap: 6, marginLeft: "auto" }}>
                          ...
                          {can("cancelMatches") && (
                            <button
                              style={{ ...s.startBtn, background: "#ef4444" }}
                              onClick={() => { if (window.confirm(`Cancel match: ${m.whiteName} vs ${m.blackName}?`)) cancelMatch(m.id); }}
                            >✕ Cancel</button>
                          )}
                        </div>
                      </div>
        ...
        {tab === "Standings" && (
          <>
            ...
            <div style={{ ...s.card, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              ...
              {can("endSeason") && standings.length > 0 && (
            ...
            {/* Tiebreaker decider approval — Head Admin only */}
            {isAdmin && unresolvedTiebreakPairs.length > 0 && unresolvedTiebreakPairs.map((pair, i) => (
              <div key={i} style={{ ...s.card, border: "1px solid rgba(239,68,68,0.35)", background: "linear-gradient(135deg, rgba(239,68,68,0.08), rgba(124,58,237,0.05))" }}>
                <div style={{ fontSize: 12, fontWeight: 800, color: "#ef4444", letterSpacing: 0.5, textTransform: "uppercase", marginBottom: 8 }}>
                  ⚔️ Tiebreaker Needed
                </div>
                <div style={{ fontSize: 13, color: T.text, marginBottom: 12, lineHeight: 1.5 }}>
                  <strong>{pair.playerAName}</strong> and <strong>{pair.playerBName}</strong> are tied for rank #{pair.rank} — they've each won once, and each won while playing White, so the usual tiebreakers can't separate them. A decider match will settle who ranks higher.
                </div>
                <div style={{ display: "flex", gap: 10 }}>
                  <button style={{ ...s.btn("#ef4444"), marginTop: 0, flex: 1 }} onClick={() => approveTiebreaker(pair)}>⚔️ Approve Decider Match</button>
                  <button style={{ ...s.btn("#374151"), marginTop: 0, flex: 1 }} onClick={() => dismissTiebreakerSuggestion(pair)}>Not Now</button>
                </div>
              </div>
            ))}
          ...
          <div style={s.card}>
            ...
            {potmAwards.length > 0 && (
              <div style={s.card}>
                <div style={s.sectionTitle}>🏅 Players of the Month</div>
                {potmAwards.slice().reverse().map(award => (
                  <div key={award.id} onClick={() => setSelectedPlayer(award.playerId)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 0", borderBottom: `1px solid ${T.border}`, cursor: "pointer" }}>
                    <Avatar name={award.playerName} size={32} photo={getPlayerPhoto(award.playerId)} />
                    <div style={{ flex: 1 }}>
                      <div style={{ fontWeight: 700, fontSize: 13, color: T.textBright }}>{award.playerName}</div>
                      <div style={{ fontSize: 11, color: T.textMuted }}>{award.monthLabel}</div>
                    </div>
                    <div style={{ fontSize: 12, color: "#f0c040", fontWeight: 700 }}>{(award.winRate * 100).toFixed(0)}%</div>
                  </div>
                ))}
              </div>
            )}
