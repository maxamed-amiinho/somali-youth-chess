...
function LiveBoard({ matchId, match, myColor, onClose, T, getPlayerPhoto, onGameEnd }) {
  ...
  return (
    <div className="live-board-no-select" style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.95)", zIndex: 10000, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: 12, overflowY: "auto" }}>
      <div style={{ width: "100%", maxWidth: 480, userSelect: "none", WebkitUserSelect: "none" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6, padding: "8px 12px", background: "#1a1f2e", borderRadius: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: "#fff", fontWeight: 600 }}>{match.blackName} ⬛</span>
              {game.status === "playing" && chess.turn() === "b" && <span style={{ color: "#22c55e", fontSize: 11, fontWeight: 700 }}>● To move</span>}
              {inCheck && chess.turn() === "b" && <span style={{ color: "#ef4444", fontSize: 11, fontWeight: 800 }}>CHECK!</span>}
            </div>
            <CapturedRow pieces={captured.byBlack} points={captured.blackPoints - captured.whitePoints > 0 ? captured.blackPoints - captured.whitePoints : 0} color="black" />
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, padding: "8px 12px", background: "#1a1f2e", borderRadius: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ color: "#fff", fontWeight: 600 }}>{match.whiteName} ⬜</span>
              {game.status === "playing" && chess.turn() === "w" && <span style={{ color: "#22c55e", fontSize: 11, fontWeight: 700 }}>● To move</span>}
              {inCheck && chess.turn() === "w" && <span style={{ color: "#ef4444", fontSize: 11, fontWeight: 800 }}>CHECK!</span>}
            </div>
            <CapturedRow pieces={captured.byWhite} points={captured.whitePoints - captured.blackPoints > 0 ? captured.whitePoints - captured.blackPoints : 0} color="white" />
          </div>
        </div>
...
export default function ChessClub() {
  ...
  return (
    <div style={s.app}>
      <style>{`@keyframes slideIn{from{opacity:0;transform:translateY(-16px)}to{opacity:1;transform:translateY(0)}} @keyframes pulse{0%,100%{opacity:1}50%{opacity:0.3}} .live-board-no-select, .live-board-no-select *{-webkit-touch-callout:none;-webkit-user-select:none;user-select:none;}`}</style>
